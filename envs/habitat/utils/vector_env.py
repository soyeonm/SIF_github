...
###
##
# Copied and pasted from new habitat
...
class VectorEnv:
    ...
    def _worker_env(
        ...
        try:
            ...
            while command != CLOSE_COMMAND:
                if command == STEP_COMMAND:
                    ...
                    if isinstance(env, habitat.RLEnv) or isinstance(
                        ...
                        if auto_reset_done and env.episode_over:
                            observations = env.reset(return_info=True)
                ...
                elif command == RESET_PHASE2:
                    observations = env.reset_phase2()
                    connection_write_fn(observations)
                ...
                elif command == SAVE_INVALID:
                    save_info = env.save_invalid(data)
                    # if auto_reset_done:
                    #     observations, info = env.reset(return_info=True)
                    connection_write_fn((save_info))
    ...
    def save_invalid(self, inputs):
        self._assert_not_closed()
        self._is_waiting = True
        for e, write_fn in enumerate(self._connection_write_fns):
            write_fn((SAVE_INVALID, inputs[e]))
        results = []
        for read_fn in self._connection_read_fns:
            results.append(read_fn())
        #breakpoint()
